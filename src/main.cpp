...
int main(int argc, char** argv){
	...
	window = SDL_CreateWindow("Kayzers", SDL_WINDOWPOS_CENTERED,
	...
	try {
		image = LoadImage("resources/img/grid_72_34.png");
	}
	...
	for(int i = 0; i < 10; ++i) {
		for (int j = 0; j < 10; ++j) {
			ApplySurface(j * tileWidth, i * tileHeight, image, renderer);
			ApplySurface(j * tileWidth + (int)(tileWidth / 2), i * tileHeight + (int)(tileHeight / 2), image, renderer);
		}
	}
